...
;;                                evil + collection + surround/commentary/snipe,
;;                                SPC leader map
...
;; -- 3.9 Evil (Vim emulation) --
;;        On by default.  `C-c t v' toggles `evil-mode' globally; `C-z' drops
;;        the current buffer into emacs-state for one-off edits.  `SPC' in
;;        normal/visual/motion is the leader (see `my/leader-map' below).
(use-package evil
  :init
  (setq evil-want-integration         t
        evil-want-keybinding          nil   ; required by evil-collection
        evil-want-C-u-scroll          t     ; vim half-page scroll
        evil-want-C-i-jump            nil   ; keep TAB for org / magit / etc.
        evil-want-Y-yank-to-eol       t
        evil-want-fine-undo           t
        evil-undo-system              'undo-redo
        evil-search-module            'evil-search
        evil-respect-visual-line-mode t
        evil-symbol-word-search       t)    ; * / # match symbols, not just words
  :bind (("C-c t v" . evil-mode)
         ("C-M-u"   . universal-argument)) ; reclaim prefix-arg from C-u scroll
  :config
  (evil-mode 1)
  ;; Terminals + REPLs should start typing-ready.
  (dolist (mode '(vterm-mode eshell-mode shell-mode term-mode comint-mode
                  eat-mode inferior-python-mode inferior-emacs-lisp-mode))
    (evil-set-initial-state mode 'insert))
  ;; Read-mostly buffers fit normal-state better.
  (dolist (mode '(messages-buffer-mode help-mode Info-mode Custom-mode
                  package-menu-mode))
    (evil-set-initial-state mode 'normal)))

(use-package evil-collection
  :after evil
  :custom (evil-collection-setup-minibuffer nil) ; keep vertico/consult intact
  :config
  ;; Curated mode list (instead of loading all ~200 integrations).  Each
  ;; entry is wrapped so a single broken integration prints a message but
  ;; doesn't abort the whole init.
  (dolist (mode '(magit dired ibuffer corfu vertico consult embark
                  flymake xref eglot help info compile grep occur
                  wgrep org org-roam vterm eshell term comint
                  package-menu calendar bookmark which-key elisp-mode
                  imenu replace tab-bar tabulated-list))
    (condition-case err
        (evil-collection-init (list mode))
      (error (message "evil-collection: skipped %s (%s)"
...
;; --- Leader (SPC in normal/visual/motion) ---
;;
;; Mirrors the most-used `C-c …' / `C-x …' bindings so Vim muscle memory and
;; classic Emacs chords both reach the same commands.  All existing bindings
;; elsewhere in this file stay untouched — this is purely additive.
(defvar my/leader-map (make-sparse-keymap)
  "Keymap for SPC leader (evil normal/visual/motion).")

(with-eval-after-load 'evil
  (evil-set-leader '(normal visual motion) (kbd "SPC"))
  (evil-define-key '(normal visual motion) 'global
    (kbd "<leader>") my/leader-map)
  ;; `,' as local-leader, conventionally used by major modes.
  (evil-set-leader '(normal visual motion) (kbd ",") t))

(defun my/leader-set (key cmd)
  "Bind KEY (relative to the leader) to CMD in `my/leader-map'."
  (define-key my/leader-map (kbd key) cmd))

;; --- Top-level shortcuts ---
(my/leader-set "SPC" #'execute-extended-command)     ; M-x
(my/leader-set "."   #'find-file)
(my/leader-set ","   #'consult-buffer)
(my/leader-set ";"   #'eval-expression)
(my/leader-set "TAB" #'mode-line-other-buffer)       ; alternate buffer
(my/leader-set "/"   #'consult-ripgrep)              ; project-wide search
(my/leader-set "x"   #'execute-extended-command)
(my/leader-set "u"   #'universal-argument)

;; --- f: files ---
(my/leader-set "ff"  #'find-file)
(my/leader-set "fs"  #'save-buffer)
(my/leader-set "fS"  #'write-file)                   ; save-as
(my/leader-set "fr"  #'consult-recent-file)
(my/leader-set "fd"  #'consult-dir)
(my/leader-set "fy"  (lambda () (interactive)
                       (when buffer-file-name
                         (kill-new buffer-file-name)
                         (message "Copied: %s" buffer-file-name))))

;; --- b: buffers ---
(my/leader-set "bb"  #'consult-buffer)
(my/leader-set "bk"  #'kill-current-buffer)
(my/leader-set "bK"  #'kill-buffer)
(my/leader-set "bn"  #'next-buffer)
(my/leader-set "bp"  #'previous-buffer)
(my/leader-set "br"  #'revert-buffer)
(my/leader-set "bi"  #'ibuffer)
(my/leader-set "bs"  #'scratch-buffer)

;; --- w: windows ---
(my/leader-set "wh"  #'windmove-left)
(my/leader-set "wj"  #'windmove-down)
(my/leader-set "wk"  #'windmove-up)
(my/leader-set "wl"  #'windmove-right)
(my/leader-set "ws"  #'split-window-below)
(my/leader-set "wv"  #'split-window-right)
(my/leader-set "wd"  #'delete-window)
(my/leader-set "wo"  #'delete-other-windows)
(my/leader-set "w="  #'balance-windows)
(my/leader-set "wm"  #'maximize-window)

;; --- p: projects ---
(my/leader-set "pp"  #'project-switch-project)
(my/leader-set "pf"  #'project-find-file)
(my/leader-set "pd"  #'project-find-dir)
(my/leader-set "pb"  #'consult-project-buffer)
(my/leader-set "pk"  #'project-kill-buffers)
(my/leader-set "ps"  #'consult-ripgrep)
(my/leader-set "pc"  #'project-compile)
(my/leader-set "p!"  #'project-shell-command)

;; --- s: search / jump ---
(my/leader-set "ss"  #'consult-line)
(my/leader-set "sS"  #'consult-line-multi)
(my/leader-set "si"  #'consult-imenu)
(my/leader-set "sI"  #'consult-imenu-multi)
(my/leader-set "sg"  #'consult-ripgrep)
(my/leader-set "sf"  #'consult-fd)
(my/leader-set "sj"  #'avy-goto-char-2)
(my/leader-set "sl"  #'avy-goto-line)

;; --- g: git / VCS ---
(my/leader-set "gg"  #'magit-status)
(my/leader-set "gb"  #'magit-blame)
(my/leader-set "gl"  #'magit-log-buffer-file)
(my/leader-set "gd"  #'magit-diff-buffer-file)
(my/leader-set "gB"  #'magit-branch-checkout)

;; --- l: LSP / eglot (mirrors existing `C-c l …') ---
(my/leader-set "la"  #'eglot-code-actions)
(my/leader-set "lr"  #'eglot-rename)
(my/leader-set "lf"  #'eglot-format-buffer)
(my/leader-set "ld"  #'eldoc)
(my/leader-set "li"  #'eglot-find-implementation)
(my/leader-set "lt"  #'eglot-find-typeDefinition)
(my/leader-set "ls"  #'consult-eglot-symbols)

;; --- e: errors / flymake ---
(my/leader-set "el"  #'flymake-show-buffer-diagnostics)
(my/leader-set "en"  #'flymake-goto-next-error)
(my/leader-set "ep"  #'flymake-goto-prev-error)

;; --- o: org ---
(my/leader-set "oa"  #'org-agenda)
(my/leader-set "oc"  #'org-capture)
(my/leader-set "ol"  #'org-store-link)
(my/leader-set "ot"  #'org-todo-list)

;; --- n: notes (org-roam) ---
(my/leader-set "nl"  #'org-roam-buffer-toggle)
(my/leader-set "nf"  #'org-roam-node-find)
(my/leader-set "ni"  #'org-roam-node-insert)
(my/leader-set "ns"  #'my/org-roam-search-text)

;; --- t: toggles ---
(my/leader-set "tv"  #'evil-mode)
(my/leader-set "tl"  #'display-line-numbers-mode)
(my/leader-set "tw"  #'visual-line-mode)
(my/leader-set "ts"  #'flyspell-mode)
(my/leader-set "tt"  #'consult-theme)

;; --- h: help (mirrors C-h) ---
(my/leader-set "hf"  #'helpful-callable)
(my/leader-set "hv"  #'helpful-variable)
(my/leader-set "hk"  #'helpful-key)
(my/leader-set "hm"  #'describe-mode)
(my/leader-set "hb"  #'embark-bindings)

;; --- q: quit ---
(my/leader-set "qq"  #'save-buffers-kill-terminal)
(my/leader-set "qr"  #'restart-emacs)

;; --- which-key labels for the SPC menus ---
